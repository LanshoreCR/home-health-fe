...
interface CreateAuditParams {
  audit: { dateRange: [Date, Date], edName: string, edId: string, businessLineId: string }
  userId: string
}

export const createAudit = async ({ audit, userId }: CreateAuditParams) => {
  try {
...
export const getAudits = async ({ userId, isAdmin }: { userId: string, isAdmin: boolean }) => {
  try {
    const params = {
      UserID: userId,
    ...
    if (response.status !== 200) return new Error('error getting audits from the database')
    const data = response.data
    const audits = data.map((item: Record<string, unknown>) => ({
...
export const getAudit = async ({ id }: { id: string }) => {
  try {
    const params = {
      PackageID: id
    }
...
export const changeAuditStatus = async ({ id, status, score }: { id: string, status: number, score: number }) => {
  try {
    const params = {
...
export const deleteAudit = async ({ packageId }: { packageId: string }) => {
  try {
    const body = {
      PackageID: packageId
    }
